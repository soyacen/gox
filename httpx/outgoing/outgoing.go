...
type options struct {
	err         error
	client      *http.Client
	middlewares []httpx.ClientMiddleware
	uri         *url.URL
	queries     url.Values
	headers     http.Header
...
type (
	MethodOptions interface {
		Middleware(middlewares ...httpx.ClientMiddleware)
...
func Middleware(middlewares ...httpx.ClientMiddleware) MethodOption {
...
func (s *options) Middleware(middlewares ...httpx.ClientMiddleware) {
	if s.err != nil {
		return
	}
...
func (s *sender) Send(ctx context.Context) (Receiver, error) {
	...
	resp, err := httpx.ClientInvoke(httpx.ClientChain(s.options.middlewares...), s.options.client, req)
	if err != nil {
		return nil, err
	}
